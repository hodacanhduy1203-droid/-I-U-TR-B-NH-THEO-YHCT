...
for (const [vId, adds] of Object.entries(additions)) {
  // FIND THE VARIANT IN FILE STRING
  // The string looks like   "sxh-th1", \n        "Vệ Khí Đồng Bệnh...
  // We need to inject into its pharmaTreatments array.
  // Let's use regex.
  const regexPharmaEnd = new RegExp(`("${vId}",[\\s\\S]*?)(],\\s*\\[)`, 'm');
  const match = content.match(regexPharmaEnd);
  
  if (match) {
    if (adds.pharma) {
      if (!match[1].includes('Đối pháp lập phương')) {
          content = content.replace(regexPharmaEnd, `$1,\n          ${adds.pharma}$2`);
      }
    }
  }

  if (adds.nonPharma) {
     const regexNonPharmaEnd = new RegExp(`("${vId}",[\\s\\S]*?],\\s*\\[[\\s\\S]*?)(\\]\\s*\\))`, 'm');
     const matchNP = content.match(regexNonPharmaEnd);
     if (matchNP) {
        if (!matchNP[1].includes('Xoa bóp bấm huyệt') && !matchNP[1].includes('Xoa bóp dưỡng sinh')) {
           content = content.replace(regexNonPharmaEnd, `$1,\n          ${adds.nonPharma}\n        $2`);
        }
     }
  }
}
...
console.log("Injected treatments for explicitly tracked variants.");
