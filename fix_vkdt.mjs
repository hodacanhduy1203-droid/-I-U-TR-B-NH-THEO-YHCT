...
const start = content.indexOf('viem-khop-dang-thap');
const variantsStart = content.indexOf('variants: [', start);
const end = content.indexOf('hoichung-co-vai-canh-tay', variantsStart);

if (start > -1 && variantsStart > -1 && end > -1) {
    let brokenSection = content.substring(variantsStart, end - 15);
    // Fix literal newlines in broken section
    // Actually simpler: just replace anything between variantsStart and end - 15 with our new string.
  ...
  },
  {
    ...
    // We replace from variantsStart to the point where `hoichung-co-vai-canh-tay` is mentioned.
    const before = content.substring(0, variantsStart);
    const after = content.substring(end);
    
    content = before + replacementVariants + after;
    fs.writeFileSync('src/data.ts', content);
    console.log('Fixed! Check compilation.');
} else {
    console.log('Something is wrong', start, variantsStart, end);
}
