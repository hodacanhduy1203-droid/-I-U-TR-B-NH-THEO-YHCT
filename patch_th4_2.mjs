...
const startIdx = content.indexOf('createVariant(\n        "dau-cot-song-th4"');
if(startIdx === -1){
   const startIdx2 = content.indexOf('createVariant(');
   // wait, find the exact one:
   const search2 = '"dau-cot-song-th4"';
   const pos = content.indexOf(search2);
   if(pos !== -1) {
      const realStart = content.lastIndexOf('createVariant(', pos);
      // find next createVariant
      const nextVariant = content.indexOf('createVariant(', pos);
      let realEnd;
      if (nextVariant !== -1) {
          realEnd = content.lastIndexOf('),', nextVariant) + 1;
      } else {
          // It's the last variant in the array
          realEnd = content.indexOf(']', pos);
          // Wait, the variant itself is closed by \)
          // Let's use substring indexOf to find the closing bracket of the variant.
          let open = 0;
      ...
      console.log('dau-cot-song-th4 not found!');
   }
}
